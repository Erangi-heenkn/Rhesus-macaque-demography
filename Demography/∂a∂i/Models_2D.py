...
def asym_mig_size_twoT(params, ns, pts):
    """
    Split with asymmetric migration and two different size-change times
    (Population 2 changes size more recently than Population 1).

    Parameters:
        nu1a: Size of population 1 just after split
        nu2a: Size of population 2 just after split
        T1:   Time of split (in 2*Na generations)
        nu1b: Size of population 1 after its size change (older event)
        nu2b: Size of population 2 after its size change (recent event)
        T1_change: Time of size change for population 1 (older)
        T2_change: Time of size change for population 2 (recent)
        m12:  Migration rate from pop2 → pop1 (2*Na*m12)
        m21:  Migration rate from pop1 → pop2 (2*Na*m21)
    """

    nu1a, nu2a, nu1b, nu2b, m12, m21, T1, T1change, T2change = params
    xx = Numerics.default_grid(pts)
    ...
    phi = Integration.two_pops(phi, xx, T1_change, nu1b, nu2a, m12=m12, m21=m21)
    
    phi = Integration.two_pops(phi, xx, T2_change, nu1b, nu2b, m12=m12, m21=m21)
